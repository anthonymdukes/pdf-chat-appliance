...
from memory.models import Base, Session, Message, DocumentInsight
import datetime
...
def test_document_insight_creation():
    SessionLocal = in_memory_db()
    with SessionLocal() as db:
        insight = DocumentInsight(doc_id="doc1", summary="Summary", key_findings="Findings", tags="tag1,tag2")
        db.add(insight)
        db.commit()
